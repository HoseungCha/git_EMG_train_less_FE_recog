...
function feat = getSampENfeat(x,winsize,wininc,datawin,dispstatus)
...
%    feat(i,:) = sum(abs(diff(curwin,2)));
   R = 0.2*std(curwin);
%    sampleEntropy(curwin,dim,r);
   for i_sig = 1 : Nsignals
       feat(i,i_sig) = sampleEntropy(curwin(:,i_sig), dim, R(i_sig),1); %%   SampEn = sampleEntropy(INPUT, M, R, TAU)
   end
   st = st + wininc;
   en = en + wininc;
end
