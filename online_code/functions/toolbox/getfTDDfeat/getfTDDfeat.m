...
function [feat,raw_sample_stamp] = getfTDDfeat(x,steps,winsize,wininc)
...
raw_sample_stamp = zeros(numwin,1);
% allocate memory
feat = zeros(numwin,Nsignals*6);
% feat = zeros(numwin,Nsignals*7)

st = 1+steps*wininc;
en = winsize+steps*wininc;
for i = 1:numwin-steps
   raw_sample_stamp(i) = en;
   curwin = x(st:en,:);
...
% Feat = [log(abs(m0)) log(abs(m0-m2)) log(abs(m0-m4)) log(abs(sparsi)) log(abs(IRF)) log(abs(WLR)) r0 ];
